---
name: pragyan-loop
description: Run one pass of pragyan's agentic analysis loop — orient on what changed, rank topics, research the hottest ones using pragyan's MCP tools + your own web research, then write a grounded report and (if warranted) propose an approval-gated insight. Use when asked to "run the pragyan loop", analyze the feed, or on a recurring /loop.
---
...
3. **Recall before researching.** For each chosen topic: `mcp__pragyan__get_dossier` (your prior
   accumulated understanding) and `mcp__pragyan__get_reports` (what you already concluded). Build
   on these — never re-research a topic from scratch if the dossier already covers it.
...
5. **Write a report.** `mcp__pragyan__submit_report` with `{ topic, title, body, opinion,
   citations }`. **Citations are mandatory** and must be real: `[{kind:"post"|"url", ref, label}]`
   — post ids from the tools above, or URLs you actually read. `opinion` is your take.

6. **Propose, don't act.** If (and only if) there's a high-conviction, *actionable* move, call
   ...
   yourself.** No actionable conclusion → no insight; a report alone is fine.

7. **Update memory.** `mcp__pragyan__update_dossier` ({ topic, state }) with your refreshed
   understanding (this is the compounding memory). Drop `mcp__pragyan__submit_lead` ({ note }) for
   threads worth pulling next pass.

## Discipline

- **Be terse and grounded.** Every claim traces to a citation or it doesn't ship.
- **Don't pad.** One strong report beats three thin ones. Often a pass produces a dossier update
  and zero insights — that's correct.
...
- **End with a one-line summary** of what you wrote (report titles, any insight proposed, dossiers
  touched) so a `/loop` history is skimmable.
